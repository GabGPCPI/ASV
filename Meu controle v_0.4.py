...
def processar_frame(frame, detector, lista_waypoints, arb_states, resolucao, fps):
    """
    ...
    limite_alcance = 5 #5 pixel (seria o tamanho do robo visto de cima +- tamanho/2)
    ...
    texto_alvo = f"Trajetoria: {len(lista_waypoints)} Waypoint(s)"
    ...
    # 3. Processar CADA tag encontrada (LÓGICA MULTI-ROBÔ)
    for tag in tags_detectadas:
        try:
            ...
            # --- B. GERENCIAR O ESTADO DO ROBÔ ---
            if tag_id not in arb_states:
                arb_states[tag_id] = {'waypoint_indice': 0, 'status': 'ativo'}  # Se for um robô novo, inicializa seu estado no dicionário
            ...
            # Se o robô já concluiu a trajetoria, apenas o desenha e pula a lógica
            if estado_arb['status'] == 'concluido':
                cv.polylines(frame, [cantos_arb_n], True, (255, 255, 255), 1) #contorno branco para robo inativo (missao concluida)
            ...
            # --- C. DETERMINAR O ALVO ATUAL DO ROBÔ ---
            indice_alvo_atual = estado_arb['waypoint_indece']
            alvo_atual = lista_waypoints[indice_alvo_atual]

            # --- D. DESENHAR O ROBÔ E SEU ALVO ---
            cv.polylines(frame, [cantos_arb_n], True, (0, 255, 0), 2) # Contorno verde (ativo)
            cv.circle(frame, centro_arb_n, 5, (0, 0, 255), -1) # Círculo vermelho no centro
            cv.putText(frame, str(tag_id), (centro_arb_n[0]+10, centro_arb_n[1]-10), 
                        font, 0.7, (0, 0, 255), 2)
            
            # --- E. CALCULAR VETOR E DISTÂNCIA ---
            ...
            # --- F. ATUALIZAR O HUD COM DADOS DESTE ARB ---
            ...
            texto_tag_alvo = f"ARB de ID:{tag_id} tem seu alvo com indice: {indice_alvo_atual} de coordenadas: ({alvo_atual[0]}, {alvo_atual[1]})"
            cv.putText(frame, texto_tag_alvo, (910, y_pos), font, font_scale, (0, 255, 255), espessura)
            y_pos += y_incremento

            texto_tag_vet = f"ARB de ID: {tag_id} tem vetor direção: [{vetor_dir[0]:.2f}, {vetor_dir[1]:.2f} pro alvo de indice: {indice_alvo_atual}]"
            cv.putText(frame, texto_tag_vet, (10, y_pos), font, font_scale, cor_texto, espessura)
            y_pos += y_incremento

            # --- G. LÓGICA DE TROCA DE WAYPOINT (SWITCHING) ---
...
def main():
    ...
    while True:
        ...
        frame_processado = processar_frame(frame, detector, lista_waypoints, arb_states, resolucao, fps) #processa o frame
