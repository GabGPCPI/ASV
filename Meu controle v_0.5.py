...
def processar_frame(frame, detector, lista_waypoints, arb_states, resolucao, fps, dt):
    """
    Recebe um frame, detecta as tags e suas informações e desenha as informações nele.
    ...
    limite_alcance = 25 #5 pixel (seria o tamanho do robo visto de cima +- tamanho/2)
    ...
    for tag in tags_detectadas:
        try:
            ...
            cantos = tag.corners # cantos[0]=ptA, [1]=ptB, [2]=ptC, [3]=ptD
            ...
            texto_tag_erro = f"Tag {tag_id} Erro Yaw: {erro_yaw_graus:.1f} deg"
            cv.putText(frame, texto_tag_erro, (10, y_pos_tags), font, font_scale, cor_texto, espessura)
            y_pos_tags += y_incremento

            texto_cmd = f"Tag {tag_id} CMD: {{{cmd_giro}{cmd_avanco}}}"
            cv.putText(frame, texto_cmd, (10, y_pos_tags), font, font_scale, (0, 165, 255), espessura) # Laranja
            y_pos_tags += y_incremento
