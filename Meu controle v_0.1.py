...
captura_de_video.set(cv.CAP_PROP_FRAME_WIDTH, 1920)
captura_de_video.set(cv.CAP_PROP_FRAME_HEIGHT, 1080)
width = captura_de_video.get(cv.CAP_PROP_FRAME_WIDTH)
height = captura_de_video.get(cv.CAP_PROP_FRAME_HEIGHT)
print(f"Resolução da câmera ajustada para {width}x{height}.")
...
nome_arquivo = f"gravacao_{timestamp}.avi"
print(f"Salvando vídeo na pasta deste .py") # Informa ao usuário onde o arquivo será salvo

grava_e_salva_video = cv.VideoWriter('output.avi', fourcc, 60.0, (width,  height))
